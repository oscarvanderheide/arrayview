...
- **smart-colormap**: Auto-selects colormap at load time based on dtype/sign: signed float/int data (vmin < 0) → RdBu_r; bool → gray; all others → gray. Implemented via `_recommend_colormap()` in `_session.py`, `recommended_colormap` field in `/metadata` response, and frontend applies it on first load (no saved state).
...
- **linked-crosshair**: In compare mode, hovering over any pane draws a dashed crosshair at the same normalized pixel position on all other visible compare panes (including diff pane). Implemented via `.cv-crosshair` overlay canvases inside each `.compare-canvas-inner`, `_drawCrossHair()`, `_clearAllCrossHairs()` in `_viewer.html`. Clears on mouseleave and on compare-mode exit.
