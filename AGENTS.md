...
- **startup-animation**: The pywebview native window now opens immediately (showing a dark `#111`/yellow `#f0c040` CSS spinner) before the FastAPI server has finished binding. A background thread in the webview subprocess polls `localhost:<port>` every 50 ms and calls `win.load_url()` once the port responds. Applies to both the in-process `view()` path and the CLI/subprocess path. `_LOADING_HTML` constant and `loading_port` parameter added to `_open_webview()`. `_open_webview_with_fallback()` and `_open_webview_cli()` both accept and forward `loading_port`. Smoke: scenario 68 (code-level checks; visual effect is native-window only).
- **compare-grid-layout**: `G` key cycles compare layout between horizontal (all panes in a row), vertical (all panes in a column), and grid (2×2, only when 3–4 arrays). New state variable `compareLayoutMode` (`null`=auto, `'horizontal'`, `'vertical'`, `'grid'`) overrides the automatic column formula in `compareScaleCanvases()` and `applyCompareLayout()`. Reset to `null` on `exitCompareMode()`. Guard: no-op in registration mode, compare-multiview, and compare-qMRI. Help overlay Display panel updated. Smoke: scenario 69.
- **picker-checkboxes**: Picker now shows a visible square checkbox (`.cp-item-cb`) on every item, always rendered (not hidden). Clicking the checkbox toggles selection without triggering open. Auto-dim: after each selection change, items whose shape differs from the first selected item's shape get `cp-item-dim` (opacity 0.4, unclickable); dimming clears when no items are selected. Shape stored as `data-shape` JSON attribute. Both sessions and files sections updated. Smoke: scenario 70.
