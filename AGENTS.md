...
- **vscode-tmux-detection**: `_find_vscode_ipc_hook()` uses two tmux fallback strategies when `TERM_PROGRAM=tmux`. Strategy 1: `tmux show-environment VSCODE_IPC_HOOK_CLI` (works if the variable is in tmux's tracked env). Strategy 2: `tmux display-message -p '#{client_pid}'` to get the PID of the tmux client process (which runs inside the VS Code terminal and directly inherits `VSCODE_IPC_HOOK_CLI`), then reads that process's environment via `ps ewwww`. This is needed because tmux pane processes are children of tmux-server (not the VS Code terminal shell), so the ancestor-walk fails; but the tmux client itself is a direct child of the VS Code terminal shell and has the hook.
