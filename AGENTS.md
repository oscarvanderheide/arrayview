...
- **vscode-tmux-detection**: `_find_vscode_ipc_hook()` now also queries `tmux show-environment VSCODE_IPC_HOOK_CLI` as a fallback when `TERM_PROGRAM=tmux`. This enables VS Code Simple Browser auto-open from tmux sessions started inside a VS Code integrated terminal, where the ancestor-process walk fails because tmux detaches the process tree.
